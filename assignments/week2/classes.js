...
class Car {
    constructor(make, model, year) {
        ...
        if (year > 2021) {
            console.log("Are you sure about the year for " + make + model + "? It seems to be awfully high!");
            this.year = NaN;
        } else {
            this.year = year;
        }
    ...
    refuel(gallons) {
        if (typeof gallons === "number") {
            ...
            if (gallons > this.fuelCapacity - this.CurrentFuelGallons) {
                ...
                this.CurrentFuelGallons = this.fuelCapacity; gallons = this.fuelCapacity;
            } else {
                this.currentFuelGallons = this.currentFuelGallons + gallons;
            }
            
            console.log("Refuel succesful, added " + gallons + "to the tank.");
        } else {
